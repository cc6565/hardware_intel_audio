                                               ...
                                               PtrSerializingCtx serializingContext)
{
    ...
    AUDIOUTILITIES_ASSERT((serializingContext->getCriterionType(name) == nullptr),
                                           ...
                                           PtrSerializingCtx serializingContext)
{
    ...
    AUDIOUTILITIES_ASSERT(serializingContext->getCriterion(name) == nullptr,
    ...
    criterion = new Criterion(name, criterionType, serializingContext->getConnector(), defaultValue);
                                           ...
                                           PtrSerializingCtx serializingContext)
{
    ...
    if (typeName == gUnsignedIntegerTypeTag) {
        ...
        AUDIOUTILITIES_ASSERT(true, ": " << tag << " type " << typeName << " not supported ");
    }
...
status_t AudioProfileTraits::deserialize(_xmlDoc */*doc*/, const _xmlNode *child, PtrElement &profile,
                                         PtrSerializingCtx /*serializingContext*/)
{
                                    ...
                                    PtrSerializingCtx /*serializingContext*/)
{
    ...
    if (device.empty()) {
        ...
        if (not convertTo<string, uint32_t>(device, mixPortConfig.deviceId)) {
            Log::Error() << __FUNCTION__ << ": No valid attribute " << Attributes::device << " found.";
            delete mixPort;
            return BAD_VALUE;
        }
    ...
    if (periodCount.empty() || !convertTo<string, uint32_t>(periodCount, mixPortConfig.periodCount)) {
    ...
    if (availMin.empty() || not convertTo<string, uint32_t>(availMin, mixPortConfig.availMin)) {
        Log::Error() << __FUNCTION__ << ": No valid attribute " << Attributes::availMin << " found.";
        delete mixPort;
        return BAD_VALUE;
    }
    ...
    if (requirePreEnable.empty() ||
        ...
        Log::Error() << __FUNCTION__ << ": No valid attribute " << Attributes::requirePreEnable
                     << " found.";
        delete mixPort;
        return BAD_VALUE;
    }
    ...
    if (requirePostDisable.empty() ||
        ...
        Log::Error() << __FUNCTION__ << ": No valid attribute " << Attributes::requirePostDisable
                     << " found.";
        delete mixPort;
        return BAD_VALUE;
    }
    mixPortConfig.dynamicChannelMapsControl = getXmlAttribute(child, Attributes::dynamicChannelMapsControl);
    mixPortConfig.dynamicFormatsControl = getXmlAttribute(child, Attributes::dynamicFormatsControl);
    mixPortConfig.dynamicRatesControl = getXmlAttribute(child, Attributes::dynamicSampleRatesControl);
    ...
    if (devicePorts.empty() || (mixPortConfig.supportedDeviceMask == 0)) {
        Log::Error() << __FUNCTION__ << ": No valid attribute " << Attributes::devicePorts << "=" << devicePorts
                     << " found.";;
        delete mixPort;
        return BAD_VALUE;
    }
    ...
    if (not channelsPolicy.empty()) {
        ...
        for (auto policyLiteral : channelsPolicyVector) {
            ...
            if (policyLiteral == Attributes::channelPolicyCopy) {
                ...
                Log::Error() << __FUNCTION__ << ": No valid attribute " << Attributes::channelsPolicy
                             << " found.";
                delete mixPort;
                return BAD_VALUE;
            }
