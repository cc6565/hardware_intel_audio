...
bool StreamRouteConfig::setCurrentSampleSpec(const SampleSpec &streamSpec)
{
    ...
    for (const auto &capabilities : mAudioCapabilities) {
        ...
        audio_channel_mask_t maskNear = capabilities.getChannelMaskNear(streamSpec.getChannelMask(), isOut);
...
{
    return (mCurrentRate != 0)? mCurrentRate : mAudioCapabilities.empty()? 0 : mAudioCapabilities[0].getDefaultRate();
}

audio_format_t StreamRouteConfig::getFormat() const
{
    return (mCurrentFormat != AUDIO_FORMAT_DEFAULT)?
                mCurrentFormat : mAudioCapabilities.empty()? AUDIO_FORMAT_DEFAULT : mAudioCapabilities[0].getFormat();
}

audio_channel_mask_t StreamRouteConfig::getChannelMask() const
{
    return (mCurrentChannelMask != AUDIO_CHANNEL_NONE) ?
                mCurrentChannelMask : mAudioCapabilities.empty()? AUDIO_CHANNEL_NONE : mAudioCapabilities[0].getDefaultChannelMask();
}
...
android::status_t StreamRouteConfig::loadChannelMaskCapabilities(AudioCapability &capability)
{
    ...
    if (mixer_ctl_get_type(ctl) != MIXER_CTL_TYPE_INT) {
        audio_utilities::utilities::Log::Error() << __FUNCTION__ << ": invalid mixer type";
        mixer_close(mixer);
        return android::BAD_VALUE;
    }
