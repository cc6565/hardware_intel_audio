...
const ChannelIndexConverter::Table ChannelIndexConverter::mTypeConversion = {
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_1), "AUDIO_CHANNEL_INDEX_MASK_1" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_2), "AUDIO_CHANNEL_INDEX_MASK_2" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_3), "AUDIO_CHANNEL_INDEX_MASK_3" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_4), "AUDIO_CHANNEL_INDEX_MASK_4" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_5), "AUDIO_CHANNEL_INDEX_MASK_5" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_6), "AUDIO_CHANNEL_INDEX_MASK_6" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_7), "AUDIO_CHANNEL_INDEX_MASK_7" },
    { static_cast<audio_channel_mask_t>(AUDIO_CHANNEL_INDEX_MASK_8), "AUDIO_CHANNEL_INDEX_MASK_8" },
};
...
const InputSourceConverter::Table InputSourceConverter::mTypeConversion = {
    ...
    { static_cast<audio_source_t>(AUDIO_SOURCE_CNT), "AUDIO_SOURCE_FM_TUNER" },
    { static_cast<audio_source_t>(AUDIO_SOURCE_CNT + 1), "AUDIO_SOURCE_HOTWORD" },
};
...
bool TypeConverter<Traits>::toEnum(const std::string &literal, T &enumVal)
{
    for (const auto &candidate : mTypeConversion) {
        if (candidate.second == literal) {
            enumVal = candidate.first;
            return true;
        }
    }
    return false;
...
{
    try {
        literal = mTypeConversion.at(enumVal);
        return true;
...
template <class Traits>
void TypeConverter<Traits>::collectionFromString(const std::string &str, Collection &collection, const char *del)
{
...
std::string TypeConverter<Traits>::maskToString(uint32_t mask, const char *del)
{
    ...
    for (const auto &candidate : mTypeConversion) {
        if ((mask & candidate.first) == candidate.first) {
            if (not formattedMasks.empty()) {
                formattedMasks += del;
            }
            formattedMasks +=  candidate.second;
        }
    }
...
std::string TypeConverter<DeviceTraits>::maskToString(uint32_t mask, const char *del)
{
    ...
    for (const auto &candidate : mTypeConversion) {
        if ((audio_is_output_devices(candidate.first) == audio_is_output_devices(mask)) &&
                ((mask & candidate.first) == candidate.first)) {
            if (not literalDevices.empty()) {
                literalDevices += del;
            }
            literalDevices +=  candidate.second;
        }
    }
